...
if __name__ == "__main__":
    ...
    if result["success"]:
        ...
        data = result["data"]
        print(f"扫码状态码: {data['scan_code']}")
        print(f"状态描述: {data['status_description']}")

        if data["scan_code"] == 0:
            print("扫码登录成功!")
            print(f"用户ID: {data.get('DedeUserID', '未获取')}")
            print(f"用户ID__ckMd5: {data.get('DedeUserID__ckMd5', '未获取')}")
            print(f"SESSDATA: {data.get('SESSDATA', '未获取')}")
            print(f"CSRF Token: {data.get('bili_jct', '未获取')}")
            print(f"b_nut Token: {data.get('b_nut', '未获取')}")
            print(f"buvid3 Token: {data.get('buvid3', '未获取')}")
            print(f"完整响应: {result}")
        else:
            print(f"详细消息: {data['scan_message']}")
    else:
