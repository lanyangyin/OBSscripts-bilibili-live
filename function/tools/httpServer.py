...
import _Input.function.tools.httpServer as InputData
...
class FlaskServer:
    ...
    def start(self):
        ...
        # 等待服务器启动
        time.sleep(1)
        print(f"服务器已启动: http://{self.host}:{self.port}")
...
# 使用示例
app = Flask(__name__, static_folder=InputData.static_folder, static_url_path='/')


@app.route('/')
def hello():
    return GlobalVariableOfData.HTML_CONTENT
...
class GlobalVariableOfData:
                ...
                class DanmuWebSocketClient {
                    ...
                    connect() {
                        try {
                            ...
                            this.socket = new WebSocket('ws://localhost:<$prot$>');
    ...
    """
GlobalVariableOfData.HTML_CONTENT = GlobalVariableOfData.HTML_CONTENT.replace("<$prot$>", "8765")

# 创建服务器实例
server = FlaskServer(app, port=5000)



# 现在有了直观的 start/stop 接口
server.start()
# ... 之后可以调用
# for i in range(1000):
#     print(i)
#     if i == 15:
#         threading.Thread(server.stop()).start()
#         try:
