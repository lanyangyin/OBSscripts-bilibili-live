"""
这个函数是一个功能强大的 URL 参数解析工具，它可以将 URL 中的查询参数和片段参数解析为结构化的字典格式。

主要功能
    - URL 参数解析：从 URL 的查询字符串（? 后面的部分）和片段（# 后面的部分）中提取参数
    - 自动 URL 解码：自动处理 URL 编码的参数（如将 %20 转换为空格）
    - 多值参数处理：支持处理同一参数名有多个值的情况
    - 智能类型转换：尝试将字符串参数转换为适当的 Python 类型
    - 参数优先级处理：查询参数优先于片段参数（当同名时）

核心特性
    1. 参数来源处理
        - 解析标准查询参数：?key=value&key2=value2
        - 解析片段中的参数：#key=value 或 #/?key=value
        - 支持同时处理查询参数和片段参数

    2. 类型自动转换
        - 函数尝试将参数值转换为最合适的 Python 类型：
        - 空字符串 → None
        - 整数字符串 → int
        - 浮点数字符串 → float
        - 百分比字符串（如 "75%"）→ 小数（0.75）
        - 布尔表示字符串 → bool（"true"/"yes"/"on"/"1" → True，反之亦然）
        - 其他字符串 → 保持原样

    3. 多值参数处理
        - 当同一参数名出现多次时，可以选择：
        - 保留所有值作为列表（handle_multiple=True）
        - 或只保留最后一个值

    4. 健壮性设计
        - 提供回退解析方案，当标准解析失败时使用手动解析
        - 处理各种边界情况（空值、特殊字符等）
        - 完整的错误处理
"""
