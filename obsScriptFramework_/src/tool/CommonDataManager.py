...
构造函数
    - python
    - def __init__(self, directory: Union[str, Path], default_data_type: str = "title")
参数：
    - directory: 数据文件存储目录
...
使用示例
    - 基本用法
        - python
        - # 创建管理器实例
        - manager = CommonDataManager(directory="data")
        - # 添加数据
        - manager.add_data("user1", "title", "我的第一个标题", maximum: int = 5)
        - manager.add_data("user1", "title", "我的第二个标题", maximum: int = 5)
        - manager.add_data("user1", "category", "科技", maximum: int = 5)
        - manager.add_data("user1", "tag", "教程", maximum: int = 5)
        - # 获取数据
